...
# 克隆代码
cd /var/www
git clone https://github.com/你的用户名/piaociyuan.git
cd piaociyuan
```
...
git archive -o piaociyuan.tar.gz HEAD

# 上传到服务器
scp piaociyuan.tar.gz root@你的服务器IP:/var/www/

# 在服务器上解压
ssh root@你的服务器IP
cd /var/www
mkdir piaociyuan && cd piaociyuan
tar -xzf ../piaociyuan.tar.gz
```
...
crontab -e
# 添加：
0 3 * * * cd /var/www/piaociyuan && ./deploy.sh backup
```
