...
`piaociyuan` 是 monorepo（Web + Mobile + shared），但两端开发节奏严重不同步：
...
| ID | 问题 | 影响 | 修复方案 |
|----|------|------|---------|
| **P0-1** | Mobile services 部分文件 `id: number`（User/Order/Ticket/Conversation/Message/Notification/Collectible/Post），与后端 UUID 字符串不匹配。**Event/Tier 的 `id: number` 是对的**（后端是 Int autoincrement） | TS 类型不真实，运行时若出现 `parseInt(id)` 之类操作必炸；后端返字符串前端按 number 用 | 通过 Phase D 引入 `@piaociyuan/shared` 类型，受影响 services 改 import |
