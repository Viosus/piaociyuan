...
import { getFollowing, followUser, unfollowUser, type FollowUser } from '../services/users';
...
export default function FollowingListScreen() {
  ...
  const loadFollowing = async (pageNum: number = page) => {
    try {
      if (pageNum === 1) {
        setLoading(true);
      } else {
  ...
  const renderEmpty = () => {
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyText}>暂无关注</Text>
      </View>
    );
  };
